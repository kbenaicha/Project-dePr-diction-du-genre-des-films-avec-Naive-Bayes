...
DATA_PATH  = BASE_DIR / "Bollywood_Movies_data" / "bollywood_merged_clean.csv"
...
def train_models():
    with st.spinner("✨  Chargement de la base Bollywood…"):
        ...
        def make_gnb():
            return Pipeline([("imp", SimpleImputer(strategy="median")), ("m", GaussianNB())])
        def make_rf():
        ...
        if predict_btn:
            try:
                ...
                with st.expander("🎞️  Films comparables dans la base"):
                    ...
                    st.dataframe(
                        sim[["Movie Name", "Budget", "Number of Screens", "Revenue", "ROI", "Success_Class"]],
...
with tab_models:
    ...
    def plot_cm(metrics, nm, task):
        ...
        cbar.ax.tick_params(colors=MUTED_C, labelsize=8)
        ax.set_xticks(np.arange(len(labels)))
        ax.set_yticks(np.arange(len(labels)))
        ax.set_xticklabels(labels, rotation=15, ha="right", fontsize=9, color=MUTED_C)
        ax.set_yticklabels(labels, fontsize=9, color=MUTED_C)
        ax.set_xlabel("Prédit", fontsize=9, color=MUTED_C)
        ax.set_ylabel("Réel",   fontsize=9, color=MUTED_C)
        tl = "Succès Commercial" if task == "success" else "Rentabilité (ROI)"
        ax.set_title(f"Matrice de confusion — {tl}", fontsize=10, fontweight="bold", color=IVORY)
        for i in range(len(labels)):
            for j in range(len(labels)):
                ax.text(j, i, cm[i, j], ha="center", va="center",
                        color=IVORY, fontweight="bold", fontsize=12)
        style_axes_bw(ax, fig)
        fig.tight_layout()
        return fig
    ...
    if "Random Forest" in fitted_models:
        ...
        for ax, task, title in [
            ...
            clf_obj = clf.named_steps["m"] if hasattr(clf, "named_steps") else clf
